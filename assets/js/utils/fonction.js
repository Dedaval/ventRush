...
export function validerEmail(email) {
	if (!email)
		return false;
    return REGEX_VALIDER_EMAIL.test(email.toLowerCase());
}

export function validerMotDePasse(motDePasse){
    if (!motDePasse)
        return false;
    return motDePasse.length >= MOT_DE_PASSE_MIN_LONGUEUR;
}

export function validerNom(nom){
    if (!nom)
        return false;
    return nom.length >= NOM_MIN_LONGUEUR;
}

export function validerPrenom(prenom){
    if (!prenom)
        return false;
    return prenom.length >= PRENOM_MIN_LONGUEUR;
}
//#endregion
